...
export async function dismissInstagramInterstitials(
  ...
  //
  // Detection: resource-id="...:id/dialog_container" is the unique wrapper; the headline
  ...
  const isFacebookSharedDialog =
    lowerXml.includes("dialog_container") &&
  ...
  if (isFacebookSharedDialog) {
    ...
    logger.warn({ serial }, "[instagram] shared-Facebook story dialog detected but live OK button was not resolved");
  }
 ...
 *
 * This is intentionally coordinate-only. The Reels viewer does not reliably
 * expose Instagram's bottom navigation through accessibility, so requiring a
 * UIAutomator node makes the exit action fail even when the icon is visibly
 * present. Keep this as a blind tap target by design.
 */
 ...
 */
export async function closeInstagramViaRecents(
  serial: string,
  dismissDirection: "left" | "up" = "left",
  onLog?: (msg: string) => void,
  swipeGesture?: {
    x1: number; y1: number; x2: number; y2: number;
    durationMinMs: number; durationMaxMs: number;
  ...
  const profileHorizontalDismiss = async (cardX: number, cardY: number): Promise<{ toX: number; durationMs: number }> => {
    if (!swipeGesture) throw new Error("Swipe Gesture Profile is required for horizontal app close");
    const profileTravel = Math.max(1, Math.abs(swipeGesture.y2 - swipeGesture.y1));
    ...
    const jitterX = Math.round((Math.random() * 2 - 1) * (Number.isFinite(swipeGesture.jitterY) ? swipeGesture.jitterY : 0));
    const startJitter = Math.round(
      (swipeGesture.startJitterMinY ?? 0) +
      Math.random() * Math.max(0, (swipeGesture.startJitterMaxY ?? 0) - (swipeGesture.startJitterMinY ?? 0)),
    );
    const startX = Math.max(0, Math.min(w - 1, Math.round(cardX + jitterX)));
    const startY = Math.max(0, Math.min(h - 1, Math.round(cardY + startJitter)));
    const toX = Math.max(0, Math.min(w - 1, Math.round(startX - profileTravel)));
    ...
    await swipe(serial, startX, startY, toX, startY, durationMs, false);
...
export async function swipeUpFromBottom(serial: string): Promise<void> {
  ...
  const y2 = Math.round(h * 0.50);
  const durationMs = 50 + Math.round(Math.random() * 50);
  logger.info({
    serial,
    requestedFrom: [x, y1],
    requestedTo: [x, y2],
...
export async function findReelActionIcons(
  ...
  // the action's resource id and state label.
  const liveSave = _findUniqueLiveActionNode(
    xml,
    [":id/save_button"],
    ["Add to Saved", "Remove from Saved"],
...
function _sleep(ms: number): Promise<void> {
  ...
  const low = Math.max(1, Math.ceil(ms * 0.8));
  const high = Math.max(low, Math.floor(ms * 1.2));
