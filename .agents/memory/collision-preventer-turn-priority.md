...
**How to apply:** Capture the HST due timestamp before clearing timer UI state, keep it immutable in the queue, sort by due time with a stable slot tie-breaker, and never release queued turns by creating a second HST timer.