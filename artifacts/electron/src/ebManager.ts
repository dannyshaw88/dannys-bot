...
async function showMyIp(){var b=document.getElementById('ipbtn');if(!b)return;b.disabled=true;var prev=b.textContent;b.textContent='...';try{var r=await fetch('https://api.ipify.org?format=json');var d=await r.json();b.textContent=d.ip||'?';b.title='Exit IP: '+d.ip+' — click to refresh';b.style.color='#059669';b.style.borderColor='#059669';}catch(e){b.textContent='IP error';b.title='Could not detect exit IP';}b.disabled=false;}
