                  ...
                  {initialLoading ? (
                    ...
                    displayFeed.slice(feedPage * 100, (feedPage + 1) * 100).map((item) => {
                      ...
                      const getCell = (col: keyof typeof DEFAULT_COL_WIDTHS) => {
                        if (col === "device_name") { const serial = item.sourceType === "phone" && item.sourceValue ? item.sourceValue.split(":")[0] : null; const dName = serial ? (deviceNameMap.get(serial) ?? serial) : "System"; return <td key={col} className="px-3 py-1.5 text-center"><span className="text-xs text-foreground">{dName}</span></td>; }
