...
**How to apply:** Keep the existing slot-level editing policy for assigned slots. Human Session Tool Copy Settings may copy Follow Sources, Random Actions profile-picture directory, Random Actions Bio spin text, and the assigned My Computer directory. My Computer remains the implicit Make a Post source; its directory is still slot-owned and must never be copied or inherited by TrustScore templates. Instagram Account Make a Post source is removed from both HST surfaces and must not return to Copy Settings or TrustScore policies. Grey out every other Copy Settings row regardless of tool toggle state, and enforce the same allow-list in the API. Keep Copy Settings visible independently of the current device's username count; its dialog should determine whether valid targets exist.

**Why:** A source-enable checkbox allowed stale saved values and previously made the slot-local directory impossible to move through the explicit HST Copy Settings flow, even though My Computer is the only supported active source.

**How to apply:** Treat old local-source enable values as compatibility data only and normalize them to true. Keep the dedicated per-slot folder-path store authoritative; allow only the directory path—not the enable field—to pass through HST Copy Settings. TrustScore template copying must continue to exclude both.
