...
export function registerMobileRoutes(httpServer: http.Server, app: Express) {
  ...
  const readTrustScoreTimer = (
    ...
    // Stable slot IDs are authoritative. A legacy numeric key is only a
    // migration fallback for installations that created timers before stable
    // slot IDs existed; slot deletion cleanup removes those old keys.
    const stableKey = trustScoreTimerKey(serial, slotIdx);
    const legacyKey = legacyTrustScoreTimerKey(serial, slotIdx);
    const raw = all[stableKey] ?? all[legacyKey];
    if (!raw) return null;
    try {
  ...
  app.get("/api/mobile/devices/:serial/slots/:slotIdx/trust-score", async (req: Request, res: Response) => {
    try {
      ...
      const { configured, scoreId } = await loadTrustScoreAssignment(serial, slotIdx);
  ...
  app.post("/api/mobile/devices/:serial/slots/:slotIdx/trust-score", async (req: Request, res: Response) => {
    try {
      ...
      const loaded = await loadTrustScoreAssignment(serial, slotIdx);
  ...
  app.post("/api/mobile/devices/:serial/slots/:slotIdx/trust-score-timer/advance", async (req: Request, res: Response) => {
    try {
      ...
      const currentTimer = readTrustScoreTimer(all, serial, slotIdx);
      if (
        !currentTimer ||
        currentTimer.scoreId !== currentScoreId ||
        trustScoreTimerRemainingMs(currentTimer) > 0
      ) {
        res.json({ ok: false, scoreId: currentScoreId });
        return;
      }
