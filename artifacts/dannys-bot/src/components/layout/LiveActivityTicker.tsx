...
import { ActivityDetailOverview } from "./ActivityDetailOverview";
...
function buildLabel(
  ...
  if (latest.sourceType === "phone") {
    ...
    return (
      <>
        <span>{account}</span>
        {detail ? <ActivityDetailOverview detail={detail} /> : <span>{actionPart}</span>}
      </>
    );
  }
  ...
  return (
    <>
      ...
      {toolLabel && <span className="text-muted-foreground/60">|</span>}
