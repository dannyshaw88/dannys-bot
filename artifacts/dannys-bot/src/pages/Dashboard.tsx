...
export function Dashboard() {
  ...
  const fetchFeed = useCallback(async (isInitial = false) => {
    try {
      ...
      const newSessionItems: FeedItem[] = newSessionRows.map((a: any) => ({
        ...
        detail: normalizeActivityDetail(a.detail),
  ...
  const { data: usbPhones } = useQuery<{ phones: { serial: string; manufacturer?: string | null; model?: string | null; marketName?: string | null }[] }>({
  ...
  const deviceNameMap = useMemo(() => {
    ...
    (usbPhones?.phones ?? []).forEach(p => {
      const label = [p.manufacturer, p.marketName || p.model].filter(Boolean).join(" ") || p.serial;
      m.set(p.serial, label);
    });
    return m;
  }, [usbPhones]);
                    ...
                    {colOrder.map(key => {
                      ...
                      return (
                        <th
                          ...
                          className={`px-3 py-4 font-bold cursor-default select-none ${isDragTarget ? "bg-primary/5 border-l-2 border-l-primary" : ""} ${(key === "trustscore" || key === "event" || key === "device_name" || key === "account_slot") ? "text-center" : ""}`}
                        >
                          {COL_LABELS[key]}
                        </th>
                      );
                  ...
                  {initialLoading ? (
                    ...
                    displayFeed.slice(feedPage * 100, (feedPage + 1) * 100).map((item) => {
                      ...
                      const getCell = (col: keyof typeof DEFAULT_COL_WIDTHS) => {
                        if (col === "device_name") { const serial = item.sourceType === "phone" && item.sourceValue ? item.sourceValue.split(":")[0] : null; const dName = serial ? (deviceNameMap.get(serial) ?? serial) : "System"; return <td key={col} className="px-3 py-1.5 text-center">{serial ? <Link href={`/mobile/farm/${encodeURIComponent(serial)}`} className="text-xs text-primary hover:underline cursor-pointer font-medium">{dName}</Link> : <span className="text-xs text-foreground">{dName}</span>}</td>; }
                        if (col === "account_slot") return <td key={col} className="px-3 py-1.5 text-center"><span className="text-xs text-foreground">{item.sourceType === "phone" && item.sourceValue?.includes(":") ? `Slot ${Number(item.sourceValue.split(":")[1]) + 1}` : "System"}</span></td>;
                        ...
                        if (col === "event") return <td key={col} className="px-3 py-3 truncate text-center"><span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider truncate inline-flex items-center gap-1 max-w-full ${style.cls}`}><span>{style.label}</span><span className="shrink-0 leading-none">{style.icon}</span></span></td>;
                         if (col === "detail") return <td key={col} className="px-3 py-3 align-middle text-foreground truncate text-xs" title={item.detail || undefined}><span className="flex h-full items-center min-w-0"><ActivityDetailOverview detail={item.detail} /></span></td>;
