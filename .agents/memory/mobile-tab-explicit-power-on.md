...
device on an explicit trigger — an on-screen Power button press or an
automation toggle being enabled — never on component mount / tab navigation
alone. Also reset any such "live" flag when the device disconnects so a
reconnect doesn't silently resume streaming.
